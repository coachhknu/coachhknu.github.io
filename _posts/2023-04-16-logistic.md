...
로지스틱 회귀 분류기에 공급할 'X_train' 데이터 세트가 준비완료
...
predict_proba 메서드는 이 경우 대상 변수(0과 1)에 대한 확률을 배열 형식으로 제공

'0은 비가 오지 않을 확률'이고 '1은 비가 올 확률'
...
y_test는 실제 클래스 레이블이고 y_pred_test는 테스트 세트에서 예측된 클래스 레이블

훈련 세트와 테스트 세트 정확도를 비교하여 과적합 여부를 확인
...
훈련 세트 정확도 점수는 0.8476이고 테스트 세트 정확도는 0.8501
두 값은 비슷함으로 과적합 문제는 없다.

로지스틱 회귀에서는 기본값 C = 1을 사용
그러나 훈련 세트와 테스트 세트의 모델 성능은 비슷함으로 과소적합의 가능성이 높다.
...
C를 100 더 높이면 테스트 세트 정확도와 훈련 세트 정확도를 증가시킨다는 것을 확인

이제 C=0.01로 설정
기본값 C=1보다 더 정규화된 모델인 경우를 조사한다
...
C=0.01로 설정하여 보다 정규화된 모델을 사용하면 교육 및 테스트 세트 정확도가 모두 기본 매개변수에 비해 감소

정확도는 0.8501
null 정확도를 비교해야할 필요성이 있다
Null 정확도는 항상 가장 빈번한 클래스를 예측하여 달성할 수 있는 정확도

테스트 세트에서 클래스 분포를 확인해야 한다.
...
가장 빈번한 클래스의 발생이 22067
따라서 22067을 총 발생 횟수로 나누어 null 정확도를 계산가능
...
null 정확도 점수는 0.7759
따라서 로지스틱 회귀 모델이 클래스 레이블을 예측하는데 효과적
...
혼동 행렬은 20892 + 3285 = 24177개의 올바른 예측'과 '3087 + 1175 = 4262개의 잘못된 예측을 보여주고 있다.


이 경우, 우리는


- 'True Positives'(실제 긍정:1 및 예측 긍정:1) - 20892


- 'True Negatives' (실제 네거티브:0 및 예측 네거티브:0) - 3285


- `False Positives` (실제 음성:0이지만 양성 예측:1) - 1175 `(유형 I 오류)`
...
<img src = "https://raw.githubusercontent.com/coachhknu/coachhknu.github.io/master/_posts/output_157_1.png">
...
Recall은 올바르게 예측된 실제 긍정의 비율을 식별
...
f1-점수는 정밀도와 재현율의 가중 조화 평균
최고의 f1-점수는 1.0이고 최악의 경우0.0
계산에 정밀도와 재현율을 포함하므로 항상 정확도 측정값보다 낮다. 
...
- 2개의 클래스(0과 1)에 해당하는 2개의 열이 존재

     - 등급 0 - 내일 비가 내리지 않을 것으로 예상되는 확률.
    
     - 등급 1 - 내일 비가 올 확률이 예측됩니다.
...
<img src = "https://raw.githubusercontent.com/coachhknu/coachhknu.github.io/master/_posts/output_175_1.png">
    


- 첫 번째 열은 확률이 0.0에서 0.1 사이인 약 15000개의 관측치가 있음

- 확률 > 0.5인 관측치가 적다.

- 그래서, 이 적은 수의 관측은 내일 비가 올 것이라고 예측

- 관측의 대부분은 내일 비가 내리지 않을 것이라고 예측.
...
ROC 곡선은 수신기 작동 특성 곡선 
ROC 곡선은 다양한 분류 모델의 성능을 보여준다
ROC 곡선은 다양한 임계값 수준에서 FPR에 대한 TPR을 나타낸다



TPR은 리콜이라고도 한다. 'TP 대 (TP + FN)'의 비율로 정의



FPR은 'FP 대 (FP + TN)'의 비율로 정의
...
<img src = "https://raw.githubusercontent.com/coachhknu/coachhknu.github.io/master/_posts/output_179_0.png">
    


ROC AUC는 Receiver Operating Characteristic - Area Under Curve를 뜻한다.
분류기 성능을 비교하는 기법
이 기술에서는 '곡선 아래 면적(AUC)'을 측정
완벽한 분류기는 ROC AUC가 1인 반면 순수 무작위 분류기는 ROC AUC가 0.5

ROC AUC는 곡선 아래에 있는 ROC 플롯의 백분율
...
원래 모델 테스트 정확도는 0.8501
GridSearch CV 정확도는 0.8507


GridSearch CV가 이 특정 모델의 성능을 향상시키는 것

로지스틱 회귀 모델 정확도 점수는 0.8501
이 모델은 호주에 내일 비가 올지 여부를 예측하는 데 매우 효과적

내일 비가 올 것이라는 관측이 적다. 
대부분의 관측은 내일 비가 내리지 않을 것이라고 예측

과적합의 징후를 보이지 않는다.

C 값을 높이면 테스트 세트 정확도가 높아지고 훈련 세트 정확도도 약간 높아진다. 
...
원래 모델 점수는 0.8476
평균 교차 검증 점수는 0.8474 따라서 교차 검증이 성능 향상으로 이어지지 않는다

원래 모델 테스트 정확도는 0.8501이고 GridSearch CV 정확도는 0.8507 GridSearch CV가 이 특정 모델의 성능을 향상시키는 것
